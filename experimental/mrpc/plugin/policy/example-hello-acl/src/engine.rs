                    ...
                    EngineTxMessage::RpcMessage(msg) => {
                        ...
                        let output: Vec<_> = iproduct!(input.iter(), self.table_acl.iter())
                            .map(|(msg, acl)| {
                                ...
                                if hello_request_name_readonly(rpc_message) == acl.name {
                                    if acl.permission != "Y" {
                                        // reject
                                        ...
                                        log::debug!("tx input reject msg:{:?}", error);
                                        ...
                                        log::debug!("tx input accept msg:{:?}", new_msg);
                        ...
                        for msg in output {
                            ...
                            match msg {
                                RpcMessageGeneral::TxMessage(msg) => {
                                    self.tx_outputs()[0].send(msg).unwrap();
                                }
