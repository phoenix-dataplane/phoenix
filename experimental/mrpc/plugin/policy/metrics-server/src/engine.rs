...
pub(crate) struct MetricsServerEngine {
    ...
    pub(crate) config: MetricsServerConfig,
}
...
impl Engine for MetricsServerEngine {
    fn activate<'a>(self: Pin<&'a mut Self>) -> BoxFuture<'a, EngineResult> {
        Box::pin(async move { self.get_mut().mainloop().await })
    }

    fn description(self: Pin<&Self>) -> String {
        "MetricsServerEngine".to_owned()
    }
    ...
    fn handle_request(&mut self, request: Vec<u8>, _cred: UCred) -> Result<()> {
        ...
        match request {
            control_plane::Request::NewConfig() => {
                // Update config
                self.config = MetricsServerConfig {};
            }
        }
        Ok(())
    }
...
impl_vertex_for_engine!(MetricsServerEngine, node);

impl Decompose for MetricsServerEngine {
...
impl MetricsServerEngine {
    pub(crate) fn restore(
        ...
        let num_rej = *local
            ...
            .downcast::<u32>()
            .map_err(|x| anyhow!("fail to downcast, type_name={:?}", x.type_name()))?;
        let engine = MetricsServerEngine {
            node,
