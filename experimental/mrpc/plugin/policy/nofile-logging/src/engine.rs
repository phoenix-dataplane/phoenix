...
pub(crate) struct NofileLoggingEngine {
    ...
    fn handle_request(&mut self, request: Vec<u8>, _cred: UCred) -> Result<()> {
        ...
        match request {
            control_plane::Request::NewConfig() => {
                self.config = NofileLoggingConfig {};
            }
        }
        Ok(())
    }
...
impl_vertex_for_engine!(NofileLoggingEngine, node);

impl Decompose for NofileLoggingEngine {
...
impl NofileLoggingEngine {
    pub(crate) fn restore(
        ...
        let config = *local
            .remove("config")
            .unwrap()
            .downcast::<NofileLoggingConfig>()
            .map_err(|x| anyhow!("fail to downcast, type_name={:?}", x.type_name()))?;
        let mut log_file = create_log_file();

        let engine = NofileLoggingEngine {
            node,
