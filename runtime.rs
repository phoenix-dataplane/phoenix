...
impl Runtime {
    ...
    pub(crate) fn mainloop(&self) -> Result<(), Error> {
        ...
        loop {
            ...
            // drive each engine
            // for (group_index, group) in self.running.borrow().iter().enumerate().cycle().take(100) {
            //     let mut group = group.borrow_mut();
            //     for (engine_index, (_eid, engine)) in group.engines.iter_mut().enumerate() {
            for (engine_index, engine) in self.running.borrow().iter().enumerate() {
            ...
            //     }
            //     // The unwrap here should be fine because runtime will be dropped later than
            //     // RuntimeManager.
                    ...
                    match submission {
                        RuntimeSubmission::NewGroup(group) => {
                            ...
                            for engine in group.engines {
                                running.push(RefCell::new(engine.1));
                            }
                        }
