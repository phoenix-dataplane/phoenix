...
/// Holds allocated data within a 1 GiB page.
///
...
///
/// # Notes
/// An object of this type will be exactly 1 GiB.
...
#[repr(C)]
pub struct HugeObjectPage<'a> {
    /// Holds memory objects.
    #[allow(dead_code)]
    data: [u8; (1024 * 1024 * 1024) - 80],

    /// Next element in list (used by `PageList`).
    next: Rawlink<HugeObjectPage<'a>>,
    prev: Rawlink<HugeObjectPage<'a>>,
...
unsafe impl<'a> Send for HugeObjectPage<'a> {}
unsafe impl<'a> Sync for HugeObjectPage<'a> {}

impl<'a> AllocablePage for HugeObjectPage<'a> {
...
impl<'a> Default for HugeObjectPage<'a> {
    fn default() -> HugeObjectPage<'a> {
        unsafe { mem::MaybeUninit::zeroed().assume_init() }
    }
}

impl<'a> fmt::Debug for HugeObjectPage<'a> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "HugeObjectPage")
    }
}
...
impl<'a, T: AllocablePage> PageList<'a, T> {
    ...
    pub(crate) fn contains(&mut self, s: *const T) -> bool {
        for slab_page in self.iter_mut() {
            if ptr::eq(slab_page as *const T, s) {
                return true;
            }
        }
