use libkoala::verbs::{MemoryRegion, WcStatus};

const SERVER_PORT: u16 = 5000;
const MSG_SIZE: usize = 65536;
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let listener =
        libkoala::cm::CmIdListener::bind(("0.0.0.0", SERVER_PORT)).expect("Listener bind failed");
    ...
    let mut recv_mr: MemoryRegion<u8> =
        pre_id.alloc_msgs(MSG_SIZE).expect("Memory registration failed!");
