...
pub fn initialize_database(uri: impl AsRef<str>) -> Result<Database> {
    ...
    let count = collections.count_documents(doc! { "hotelId": "4" }, None)?;
    if count == 0 {
        let hotel = Point {
            id: "4".to_string(),
            lat: 37.7936,
    ...
    let count = collections.count_documents(doc! { "hotelId": "5" }, None)?;
    if count == 0 {
        let hotel = Point {
            id: "5".to_string(),
            lat: 37.7831,
    ...
    let count = collections.count_documents(doc! { "hotelId": "6" }, None)?;
    if count == 0 {
        let hotel = Point {
            id: "6".to_string(),
            lat: 37.7863,
    ...
    for i in 7..=80 {
        ...
        let count = collections.count_documents(doc! { "hotelId": &*hotel_id }, None)?;
