...
macro_rules! impl_vertex_for_engine {
    ($engine:ident, $node:ident $($vars:tt)*) => {
        impl$($vars)* phoenix::engine::datapath::graph::Vertex for $engine$($vars)* {
            #[inline]
            fn tx_inputs(&mut self) -> &mut Vec<phoenix::engine::datapath::graph::TxIQueue> {
                &mut self.$node.tx_inputs
            }
            fn tx_outputs(&mut self) -> &mut Vec<phoenix::engine::datapath::graph::TxOQueue> {
                &mut self.$node.tx_outputs
            }
            fn rx_inputs(&mut self) -> &mut Vec<phoenix::engine::datapath::graph::RxIQueue> {
                &mut self.$node.rx_inputs
            }
            fn rx_outputs(&mut self) -> &mut Vec<phoenix::engine::datapath::graph::RxOQueue> {
