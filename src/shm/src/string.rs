...
impl<A: ShmAllocator + Default> String<A> {
    ...
    ///
    /// [`Cow<'a, str>`]: std::borrow::Cow "borrow::Cow"
    ///
    /// # Examples
    ///
    /// Basic usage:
    ...
    pub fn from_utf16(v: &[u16]) -> Result<Self, FromUtf16Error> {
        // TODO(cjr): This function is 2x slower than its std version.
        // Construct the FromUtf16Error through mem::transmute.
    ...
    #[inline]
    pub fn from_utf8(vec: Vec<u8, A>) -> Result<Self, FromUtf8Error> {
        let _ = std::string::String::from_utf8(vec.as_slice().to_owned())?;
        Ok(String { vec })
    }
}

impl<A: ShmAllocator> String<A> {
    ...
    where
        F: FnMut(char) -> bool,
    {
        struct SetLenOnDrop<'a, A: ShmAllocator = System> {
            s: &'a mut String<A>,
            idx: usize,
            del_bytes: usize,
        }

        impl<'a, A: ShmAllocator> Drop for SetLenOnDrop<'a, A> {
            fn drop(&mut self) {
...
impl<A: ShmAllocator + Default + Clone> String<A> {
...
#[cfg(not(no_global_oom_handling))]
impl<A: ShmAllocator + Clone> Clone for String<A> {
    fn clone(&self) -> Self {
        String {
            vec: self.vec.clone(),
        }
...
#[cfg(not(no_global_oom_handling))]
impl<A: ShmAllocator + Default> FromIterator<char> for String<A> {
    fn from_iter<I: IntoIterator<Item = char>>(iter: I) -> String<A> {
        let mut buf = String::<A>::new();
        buf.extend(iter);
        buf
    }
}
...
#[cfg(not(no_global_oom_handling))]
impl<'a, A: ShmAllocator + Default> FromIterator<&'a char> for String<A> {
...
#[cfg(not(no_global_oom_handling))]
impl<'a, A: ShmAllocator + Default> FromIterator<&'a str> for String<A> {
...
#[cfg(not(no_global_oom_handling))]
impl<A: ShmAllocator + Default> FromIterator<String<A>> for String<A> {
...
#[cfg(not(no_global_oom_handling))]
impl<A: ShmAllocator + Default> FromIterator<Box<str>> for String<A> {
...
#[cfg(not(no_global_oom_handling))]
impl<A: ShmAllocator> Extend<char> for String<A> {
...
#[cfg(not(no_global_oom_handling))]
impl<'a, A: ShmAllocator> Extend<&'a char> for String<A> {
...
#[cfg(not(no_global_oom_handling))]
impl<'a, A: ShmAllocator> Extend<&'a str> for String<A> {
...
#[cfg(not(no_global_oom_handling))]
impl<A: ShmAllocator> Extend<Box<str>> for String<A> {
...
#[cfg(not(no_global_oom_handling))]
impl<A: ShmAllocator> Extend<String<A>> for String<A> {
...
#[cfg(not(no_global_oom_handling))]
impl<'a, A: ShmAllocator> Extend<Cow<'a, str>> for String<A> {
...
/// ```
impl<'a, 'b, A: ShmAllocator> Pattern<'a> for &'b String<A> {
...
impl_eq! { [A: ShmAllocator] String<A>, str }
impl_eq! { [A: ShmAllocator] String<A>, &'a str }
#[cfg(not(no_global_oom_handling))]
...
impl<A: ShmAllocator> ops::Deref for String<A> {
    type Target = str;
...
impl<A: ShmAllocator> ops::DerefMut for String<A> {
    #[inline]
...
#[cfg(not(no_global_oom_handling))]
impl<A: ShmAllocator + Default> FromStr for String<A> {
    type Err = core::convert::Infallible;
    #[inline]
    fn from_str(s: &str) -> Result<String<A>, Self::Err> {
        Ok(String::from(s))
    }
}

impl<A: ShmAllocator> AsRef<str> for String<A> {
    #[inline]
    fn as_ref(&self) -> &str {
        self
    }
...
impl<A: ShmAllocator> AsMut<str> for String<A> {
    #[inline]
...
impl<A: ShmAllocator> AsRef<[u8]> for String<A> {
    #[inline]
    fn as_ref(&self) -> &[u8] {
        self.as_bytes()
    }
...
#[cfg(not(no_global_oom_handling))]
impl<A: ShmAllocator + Default> From<&str> for String<A> {
    ...
    fn from(s: &str) -> Self {
        Self {
            vec: Vec::from_iter(s.bytes()),
        }
    }
}

#[cfg(not(no_global_oom_handling))]
impl<A: ShmAllocator + Default> From<&mut str> for String<A> {
...
#[cfg(not(no_global_oom_handling))]
impl<A: ShmAllocator + Default> From<&String<A>> for String<A> {
...
impl<A: ShmAllocator + Default> From<std::string::String> for String<A> {
    /// Converts a `std::string::String` into a [`String`].
    ///
    /// This clones `s` and returns the clone.
    #[inline]
    fn from(s: std::string::String) -> String<A> {
        Self::from(s.as_str())
    }
}
...
#[cfg(not(no_global_oom_handling))]
impl<'a, A: ShmAllocator + Default> From<Cow<'a, str>> for String<A> {
...
#[cfg(not(no_global_oom_handling))]
impl<'a, A: ShmAllocator> From<&'a String<A>> for Cow<'a, str> {
...
impl<A: ShmAllocator> From<String<A>> for Vec<u8, A> {
...
///
/// [`drain`]: String::drain
pub struct Drain<'a, A: ShmAllocator = System> {
...
unsafe impl<A: ShmAllocator> Sync for Drain<'_, A> {}
unsafe impl<A: ShmAllocator> Send for Drain<'_, A> {}

impl<A: ShmAllocator> Drop for Drain<'_, A> {
    fn drop(&mut self) {
        unsafe {
...
#[cfg(not(no_global_oom_handling))]
impl<A: ShmAllocator + Default> From<char> for String<A> {
