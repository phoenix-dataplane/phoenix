...
impl RpcAdapterEngine {
    pub(crate) unsafe fn restore(
        ...
        let dp_tx = *local
            .remove("dp_tx")
            .unwrap()
            .downcast_unchecked::<crossbeam::channel::Sender<EngineRxMessage>>();
        let dp_rx = *local
            .remove("dp_rx")
            .unwrap()
            .downcast_unchecked::<crossbeam::channel::Receiver<EngineTxMessage>>();
        ...
        let engine = RpcAdapterEngine {
            ...
            _mode: mode,
            indicator: None,
            // control message buffer should not be used when engine is detached
...
impl RpcAdapterEngine {
    ...
    fn check_input_queue(&mut self) -> Result<Status, DatapathError> {
        ...
        while let Some(msg) = self.local_buffer.pop_front() {
            ...
            // V1.0, only support standard send.
                    ...
                    match wc.opcode {
                        WcOpcode::Send => {
                            ...
                            if wc.wc_flags.contains(WcFlags::WITH_IMM) {
                                ...
                                if wc.wr_id == u64::MAX {
                                    // control message
                                    // there is always at most one outstanding control message
