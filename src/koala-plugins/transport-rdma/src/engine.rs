...
impl TransportEngine {
        ...
        match ret {
            ...
            Ok(req) => {
                ...
                match result {
                    ...
                    Err(e) => {
                        // better to tracing the error here, in case sometimes the customer does
                        // not receive the error
                        tracing::error!("process_cmd error: {}", e);
    ...
    async fn process_cmd(&mut self, req: &cmd::Command) -> Result<cmd::CompletionKind, Error> {
        ...
        match req {
            ...
            Command::Listen(cmid_handle, backtracing) => {
                self.ops.listen(*cmid_handle, *backtracing)?;
