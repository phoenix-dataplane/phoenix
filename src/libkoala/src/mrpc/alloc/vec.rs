...
impl<T> SpecializedDrop for Vec<T, BackendOwned> {
    fn drop(&mut self) {
        // TODO(wyj)
        return;
    }
...
impl<T, O: AllocOwner> SpecializedDrop for Vec<T, O> {
...
impl<T, O: AllocOwner> ops::Deref for Vec<T, O> {
    type Target = [T];
...
impl<T: Hash, O: AllocOwner> Hash for Vec<T, O> {
    #[inline]
...
impl<T, I: SliceIndex<[T]>, O: AllocOwner> Index<I> for Vec<T, O> {
...
impl<T: fmt::Debug, O: AllocOwner> fmt::Debug for Vec<T, O> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        fmt::Debug::fmt(&**self, f)
    }
}

impl<T, O: AllocOwner> AsRef<Vec<T, O>> for Vec<T, O> {
    fn as_ref(&self) -> &Vec<T, O> {
        self
    }
}
...
impl<T, O: AllocOwner> AsRef<[T]> for Vec<T, O> {
    fn as_ref(&self) -> &[T] {
        self
    }
}
...
/// A draining iterator for `Vec<T, AppOwned>`.
///
...
/// A splicing iterator for `Vec<T, AppOwned>`.
///
...
/// An iterator produced by calling `drain_filter` on Vec<T, AppOwned>.
#[derive(Debug)]
